  ...
  public class Node extends NodeList {
    ...
    protected class SubNodeAdder {
      ...
      protected void addSubTasks() throws IOException, StorageException {
        for (RelativeSubTask relativeSubTask : task.subTasks) {
          try {
            Optional<Task> def =
                preloader.getOptionalTask(
                    taskExpressionFactory.create(relativeSubTask.file(), relativeSubTask.value()));
            if (def.isPresent()) {
      ...
      protected void addSubTasksFiles() {
        for (RelativeSubTask relativeSubTask : task.subTasksFiles) {
          try {
            addPreloaded(
                preloader.getTasks(
                    FileKey.create(
                        relativeSubTask.file().branch(),
                        resolveTaskFileName(relativeSubTask.value()))));
      ...
      protected void addSubTasksExternals() throws StorageException {
        for (RelativeSubTask relativeSubTask : task.subTasksExternals) {
          try {
            External ext =
                taskConfigFactory
                    .getTaskConfig(relativeSubTask.file())
                    .getExternal(relativeSubTask.value());
            if (ext == null) {
              addInvalidNode();
            } else {
      ...
      protected void addSubTasksFactoryTasks()
          throws IOException, StorageException, ConfigInvalidException {
        for (RelativeSubTask relativeSubTask : task.subTasksFactories) {
          TasksFactory tasksFactory =
              taskConfigFactory
                  .getTaskConfig(relativeSubTask.file())
                  .getTasksFactory(relativeSubTask.value());
