  ...
  public class Node extends NodeList {
    ...
    protected class SubNodeAdder {
      ...
      protected void addSubTasks() throws IOException, StorageException {
        for (SourceSubTask sourceSubTask : task.subTasks) {
          try {
            Optional<Task> def =
                preloader.getOptionalTask(
                    taskExpressionFactory.create(
                        sourceSubTask.getSourceFile(), sourceSubTask.getValue()));
            if (def.isPresent()) {
      ...
      protected void addSubTasksFiles() {
        for (SourceSubTask sourceSubTask : task.subTasksFiles) {
          try {
            addPreloaded(
                preloader.getTasks(
                    FileKey.create(
                        sourceSubTask.getSourceFile().branch(),
                        resolveTaskFileName(sourceSubTask.getValue()))));
      ...
      protected void addSubTasksExternals() throws StorageException {
        for (SourceSubTask sourceSubTask : task.subTasksExternals) {
          try {
            External ext =
                taskConfigFactory
                    .getTaskConfig(sourceSubTask.getSourceFile())
                    .getExternal(sourceSubTask.getValue());
            if (ext == null) {
              addInvalidNode();
            } else {
      ...
      protected void addSubTasksFactoryTasks()
          throws IOException, StorageException, ConfigInvalidException {
        for (SourceSubTask sourceSubTask : task.subTasksFactories) {
          TasksFactory tasksFactory =
              taskConfigFactory
                  .getTaskConfig(sourceSubTask.getSourceFile())
                  .getTasksFactory(sourceSubTask.getValue());
