            ...
            Optional<Task> def =
                preloader.getOptionalTask(
                    taskExpressionFactory.create(
                        configSourcedValue.getSourceFile(), configSourcedValue.getValue()));
            if (def.isPresent()) {
        ...
        for (ConfigSourcedValue configSourcedValue : task.subTasksFiles) {
          try {
            addPreloaded(
                preloader.getTasks(
                    FileKey.create(
                        configSourcedValue.getSourceFile().branch(),
                        resolveTaskFileName(configSourcedValue.getValue()))));
      ...
      protected void addSubTasksFactoryTasks()
        ...
        for (ConfigSourcedValue configSourcedValue : task.subTasksFactories) {
          TasksFactory tasksFactory =
              taskConfigFactory
                  .getTaskConfig(configSourcedValue.getSourceFile())
                  .getTasksFactory(configSourcedValue.getValue());
