...
"is:true_@PLUGIN@" submit requirement or the special "True" keyword may
be used as any query definition to indicate an always matching query. The
...
this to "is:true_@PLUGIN@" or "True" is useful for defining blocking criteria
that do not actually have a task to execute.
...
one subtask is applicable. Setting this to "is:true_@PLUGIN@" or "True"
is useful for defining informational tasks that are not really expected
to execute.
