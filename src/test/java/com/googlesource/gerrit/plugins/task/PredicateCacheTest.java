...
public class PredicateCacheTest extends LightweightPluginDaemonTest {
  ...
  public void setUp() {
    taskPluginConfiguration =
        new com.googlesource.gerrit.plugins.task.TaskPluginConfiguration(
            PLUGIN, config, pluginConfigFactory, allProjectsNameProvider);
    predicateCache =
        new com.googlesource.gerrit.plugins.task.PredicateCache(
            taskPluginConfiguration, currentUser, srcqb);
  }

  @Test
  public void isCacheableByBranchTest() throws Exception {
    assertThat(predicateCache.isCacheableByBranch("project:foo", true)).isTrue();
    assertThat(predicateCache.isCacheableByBranch("branch:bar", true)).isTrue();

    assertThat(predicateCache.isCacheableByBranch("project:^foo", true)).isTrue();
    assertThat(predicateCache.isCacheableByBranch("branch:^bar", true)).isTrue();

    assertThat(predicateCache.isCacheableByBranch("project:foo branch:bar", true)).isTrue();
    assertThat(predicateCache.isCacheableByBranch("project:^foo branch:bar", true)).isTrue();
    assertThat(predicateCache.isCacheableByBranch("project:foo branch:^bar", true)).isTrue();

    assertThat(predicateCache.isCacheableByBranch("topic:baz", true)).isFalse();
    assertThat(predicateCache.isCacheableByBranch("label:Code-Review+1", true)).isFalse();
    assertThat(predicateCache.isCacheableByBranch("topic:baz project:foo", true)).isFalse();
  }
}
