...
public class TaskReferenceTest extends TestCase {
  ...
  protected static TaskKey getTaskFromReference(FileKey file, String expression) {
    ...
    Mockito.when(pluginConfigFactory.getGlobalPluginConfig(pluginName)).thenReturn(new Config());
    try {
      return new TaskReference(
              new TaskKey.Builder(
                  ...
                  new TaskPluginConfiguration(
                      pluginName, new Config(), pluginConfigFactory, allProjectsNameProvider)),
              expression)
          .getTaskKey();
