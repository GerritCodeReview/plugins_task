...
usage() { # [error_message]
    cat <<-EOF
Usage:
    $MYPROG [--task-plugin-jar|-t <FILE_PATH>] [--names-factory-provider-plugin-jar|-f <FILE_PATH>] [--gerrit-war|-g <FILE_PATH>]
    ...
    --names-factory-provider-plugin-jar|-f    path to names-factory-provider plugin JAR file. It can be generated using 'bazel build names-factory-provider'
